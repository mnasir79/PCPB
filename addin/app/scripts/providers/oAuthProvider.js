  ...
  provider('oAuth', function() {
    ...
    this.$get = function($q, $http, $window, $rootScope) {
      ...
      /*if (requiredAndMissing.length) {
        throw new Error('oAuthProvider is insufficiently configured. Please configure the following options using oAuthProvider.extendConfig: ' + requiredAndMissing.join(', '));
      }

      if (!config.clientId) {
        throw new Error('clientId needs to be configured using oAuthProvider.');
      }*/
